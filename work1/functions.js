...
function oklad() {
    ...
    var oklad, days, factdays, result;

    oklad = +prompt("Введите оклад");

    if (oklad) {
        days = +prompt("Введите количество дней");
        if (days) {
            factdays = +prompt("Введите количество отработанных дней");
            if (factdays) {
                result = ((oklad/days) * factdays * 0.87).toFixed(2);
                alert("Ваша зарплата: " + result);
            } else alert("Не введены количество отработанных дней");
        } else alert("Не введены количество дней");
    } else alert("Не введен оклад");
...
function language() {
    ...
    var menu = prompt("Выберите язык меню: \n 1. Русский \n 2. English \n 3. Deutch");
